...
def create_schema():
    ...
    return Schema(id=ID, year=ID, title=TEXT, event_type=TEXT,
                  pdf_name=TEXT(stored=True), abstract=TEXT, paper_text=TEXT)
    ...
    with open(csv_file_name) as csvfile:
        ...
        for item in reader:
            writer.add_document(
                ...
                paper_text=str(item['paper_text']))
...
def search(query_str, search_field="title", index_name="index"):
    ...
    myquery = QueryParser(search_field, ix.schema).parse(str(query_str))
